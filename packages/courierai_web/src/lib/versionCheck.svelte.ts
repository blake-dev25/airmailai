...
class VersionCheck {
    ...
    notice: UpdateNotice | null = $derived.by(() => {
        if (this.extStatus === 'updated') {
            return {
                message:
                    'The CourierAI extension just updated. Refresh to reconnect.',
                refresh: true,
            };
        }
        if (this.webUpdateAvailable) {
            return {
                message:
                    'A new version of CourierAI is available. Refresh to get it.',
                refresh: true,
            };
        }
        if (this.extStatus === 'outdated') {
            return {
                message:
                    'A CourierAI extension update is rolling out. Chrome will install it automatically within a few hours, or restart your browser to get it now.',
                refresh: false,
            };
        }
        return null;
