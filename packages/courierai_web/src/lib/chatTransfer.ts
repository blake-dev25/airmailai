...
export type ChatExportFormat = 'md' | 'courierai' | 'lmstudio' | 'sillytavern';
...
function attachmentNames(msg: CourierAIMessage): string[] {
    const names: string[] = [];
...
function conversationTokenCount(messages: CourierAIMessage[]): number {
...
export function buildCourierAIJsonExport(
    info: TransferChatInfo,
    messages: CourierAIMessage[]
): string {
    return JSON.stringify(courieraiChatJson(info, messages), null, 2);
}

function courieraiChatJson(
    info: TransferChatInfo,
    messages: CourierAIMessage[]
): Record<string, unknown> {
    return {
...
export function buildBackupExport(chats: TransferChatEntry[]): string {
    return (
        chats
            .map((c) =>
                JSON.stringify(courieraiChatJson(c.info, c.messages), null, 2)
            )
            .join('\n---\n') + '\n'
    );
}
